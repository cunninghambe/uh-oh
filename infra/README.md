...
| `UH_OH_ALERT_LOCAL_TZ`      | `America/New_York` | IANA zone for the local half of alert timestamps. Chat alerts (the Discord one-liners) render every time as UTC followed by local time, e.g. `2026-08-29 06:31 UTC (02:31 EDT)`, so an alert lines up with the box's journal without a mental offset. Set it to the zone your logs are written in; `UTC` drops the parenthetical. A zone ICU does not recognise is logged at error and the default is used — the server still boots.                                                               |
