...
- Webhook URLs are SSRF-validated at save AND dispatch time: http/https only; no userinfo credentials (`http://user:pass@host/` is rejected — they would be stored in plaintext and echoed back to every narrower read scope); literal loopback/private/link-local/metadata IPs rejected, plus CGNAT `100.64.0.0/10`, `192.0.0.0/24`, benchmarking `198.18.0.0/15`, multicast `224.0.0.0/4`, reserved `240.0.0.0/4` (which covers `255.255.255.255`), IPv6 site-local `fec0::/10`, and the whole IPv4-compatible `::/96` (the URL parser normalizes `::127.0.0.1` to `::7f00:1`, so the range is blocked wholesale rather than by inspecting an embedded IPv4); redirects refused (DNS-rebinding via hostnames is a known v0.1 gap). One validator governs every outbound target: project webhooks and (§24) uptime-probe URLs. No port restriction — monitor CRUD is JWT-only and legitimate internal health endpoints use odd ports
- Payload size cap (1 MB) — SDK trims breadcrumbs then context then retries once on 413
- Retention: a daily in-process job prunes events older than `UH_OH_RETENTION_DAYS` (default 90; 0 disables), terminal webhook dispatches older than 7 days, and (§23) the `issue_annotations` / `fix_attempts` of DEAD issues — those left with no event rows after the event prune; issues themselves are kept
...
| 12  | Mis-grouping                               | SDK `setFingerprint` is the v0.1 escape hatch. No server-side merge UI. (Issue merge shipped in v0.9 — §24.)                                                                                                                                      |
...
**Annotations.** Migration 0007 adds `issue_annotations` (id, issue_id FK cascade, author TEXT ≤128 BYTES default `'agent'`, kind TEXT ∈ `note|root_cause|fix_plan|verification|system` default `'note'`, body TEXT ≤16KB, created_at). `POST /api/issues/:id/annotations` `{ body, kind?, author? }` → 201 `{ annotation }` (400 on bad kind or over-cap author, 413 over the body cap, 409 `annotation_limit` past **500 client annotations per issue**); `GET /api/issues/:id/annotations?limit=&offset=` → `{ annotations, total }` newest-first. Both caps are counted in BYTES, not characters, and both the REST route and the MCP `annotate_issue` path enforce every cap identically (the tool schema caps characters, so the backend re-checks bytes and the per-issue count) — an agent cannot dodge a cap by choosing the other entry point. The per-issue cap bounds the one agent-writable table that the retention job leaves alone while the issue is alive; server-written `kind:'system'` audit rows are exempt from it, so flooding can never suppress the audit trail. The server writes `kind: 'system'` rows on every fix-attempt transition (audit trail). The issue bundle gains `annotations` (last 10, newest first), and the bundle's truncation order becomes: source-context lines first, then breadcrumbs, then annotations oldest-first — annotations are the most protected content; the always-present `truncated` flags extend to them. Dashboard: annotation timeline on issue detail (kind badges, whitespace-preserved bodies).

**Fix attempts.** Migration 0007 adds `fix_attempts` (id, issue_id FK cascade, pr_url TEXT ≤512 and `/^https?:\/\//i`, commit_sha nullable same regex as releases, state TEXT ∈ `filed|deployed|verified|failed`, created_at, deployed_at nullable, updated_at, UNIQUE(issue_id, pr_url)). `POST /api/issues/:id/fix-attempts` `{ prUrl, commitSha? }` upserts by (issue, prUrl) into state `filed`; a non-http(s) `prUrl` is a 400 at both the REST route and the MCP `record_fix_attempt` path (schema regex plus a backend re-check), because the dashboard renders it as an `<a href>` and the dashboard JWT lives in localStorage — a `javascript:` URL there would be full admin compromise. `PATCH /api/fix-attempts/:id` `{ state?, commitSha? }` permits `filed→deployed`, `filed→failed`, `deployed→failed`; `verified` is system-set only; anything else 400. Marking `deployed` stamps `deployed_at` and system-sets the issue `resolved` when it is currently `open|regressed` (re-arming §18 regression detection). From there the verdict is deterministic:
...
Metrics `uh_oh_fix_verified_total`, `uh_oh_fix_failed_total`. Issue detail and bundle expose `fixAttempts` (newest first). Dashboard: fix-attempts panel on issue detail (state pills, PR links, short SHAs linked to `<repoUrl>/commit/<sha>` when repo_url is https). The panel builds an `href` only for an http(s) `prUrl` and renders anything else as plain text — the same "null means plain text" contract as the commit link, and defence in depth for rows written before the server-side scheme check.
...
## 24. v0.9 addendum — release health, console breadcrumbs, uptime probes, issue merge

Closes the remaining "Sentry does this and we want it" gaps worth having for internal tooling, plus the e2e coverage the v0.8 dashboard surfaces shipped without. Same posture as always: deterministic server, no LLM calls, features degrade gracefully when absent.

**Console breadcrumbs (`@uh-oh/js` 0.6.0).** Opt-in `init({ consoleBreadcrumbs: true })` (browser + Node): wraps `console.debug/log/info/warn/error` (guarded install, original always invoked, fully restored on `close()`), recording each call as a breadcrumb `{ category: 'console', level: debug→debug, log/info→info, warn→warning, error→error, message }`. Message = arguments stringified (primitives via `String`, objects via `JSON.stringify` with a `[unserializable]` fallback, space-joined, 500-char cap — inside the schema's 1024 limit), no `data` payload. A re-entrancy guard means a console call made while recording (including uh-oh's own debug logging) is passed through without recording — no recursion, ever. Everything rides the existing breadcrumb pipeline: no wire-format change, no server change, already rendered by the dashboard and carried in the issue bundle. The RN SDK is unchanged (manual `addBreadcrumb` exists; RN auto-console capture is out of scope).

**Usage release attribution.** `usage_events` gains nullable `release` (migration 0008); usage ingest accepts an optional per-event `release` string (≤128 chars, invalid → event dropped not batch); the client stamps the CANONICAL release identity `version+build` — the exact strings its envelope `release` parses to, so a buildless `init` release stamps as `1.2.3+0` — on every usage event automatically. Attribution is by equality against the release row's `version || '+' || build`; a bare version never attributes. The summary endpoint is unchanged; release health consumes it.

**Release health.** `GET /api/projects/:id/release-health?days=N` (N 1..90, default 30; JWT, read token, agent token) → `{ releases, totals }`. `releases`: every release with events in the window (≤20, ordered `lastEventAt` desc), each `{ id, version, build, platform, commitSha, events, fatalEvents, distinctIssues, firstEventAt, lastEventAt, pageviews, crashesPer1kPageviews }` — `pageviews` counts usage pageviews attributed to that release; `crashesPer1kPageviews` = `events / pageviews × 1000` rounded to 1 decimal, `null` when `pageviews` is 0 (analytics off, non-web, or unattributed). `totals`: the same five numeric fields project-wide (every event and every pageview in the window, attributed or not, and the ratio of the two). Deterministic SQL over existing indexes. MCP tool `get_release_health` (scope `read`; tool count 17 → 18). Dashboard: a Release health section on the project page (table, ratio badges, hidden gracefully when the endpoint is absent — the §21 pattern).

**Uptime probes.** Monitors gain `kind: 'checkin' | 'http'` (migration 0008: `kind` default `'checkin'`, plus `url`, `timeout_ms`, `last_probe_at`, `last_probe_status`, `consecutive_failures`). An http monitor is created via the existing JWT monitor CRUD `{ kind: 'http', slug, url, intervalMinutes (min 1), timeoutMs? (default 10000, cap 30000) }`; `kind` is immutable after create (400); check-in pings against an http monitor return 409. `url` is SSRF-validated at save AND probe time exactly like webhook URLs (scheme, no userinfo credentials, literal-IP rejection over the full §2 blocklist, DNS re-check; redirects refused). The existing 60s monitor sweep also probes http monitors due by `last_probe_at + interval`: GET with the configured timeout, `User-Agent: uh-oh-uptime/1`, response body never read but always CANCELLED so the socket is released immediately rather than at GC; success = status 200–399. Two consecutive failures flip `ok → missed` and dispatch `monitor.missed` (payload gains `probe: { status?, error? }`); the first success flips back with `monitor.recovered` (episode semantics identical to check-in monitors). In-flight probes capped at 5 per sweep tick, remainder picked up next tick. Metric `uh_oh_uptime_probe_failures_total`. Dashboard: monitor create/edit knows both kinds; http rows show URL + last probe status. MCP `list_monitors` exposes `kind`, `url`, `lastProbeStatus`. The one new outbound-call surface is the probe, governed by the same egress rules as webhooks.

**Issue merge.** `POST /api/issues/:id/merge { into }` (JWT only — deliberately not agent-scoped; both issues in the same project, target ≠ source, target not itself merged; violations 400/404). In one transaction: all source events re-point to the target (`issue_id`), target counters combined ADDITIVELY from the two issue rows (`event_count` summed, `first_seen` min, `last_seen` max — a row recount would drop the rate-limited counts edge case 4 accumulates without event rows); the source's fingerprint — and any `fingerprint_aliases` rows already pointing at the source — become aliases of the target (migration 0008 table `fingerprint_aliases (project_id, fingerprint, issue_id, created_at, UNIQUE(project_id, fingerprint))`); source annotations and fix attempts move to the target (with one system annotation `merged from "<source title>"`); source spike columns clear; the source flips to system-set terminal status `'merged'` (excluded from every list/filter default, only visible via explicit `status=merged`; PATCH rejects it; a merged issue's detail page links to the target). Ingest consults `fingerprint_aliases` before `issues.fingerprint` — an alias hit routes the event to the target issue with normal count/regression semantics. No unmerge in v0.9 (re-merge chains are prevented, not unwound). Metric `uh_oh_issues_merged_total`. Dashboard: a Merge action on issue detail — a modal offering the `/similar` results as click-to-select targets (selection fills the target field; a single explicit Merge button is the sole confirmation, because merge is irreversible) plus a free issue-id field, navigating to the target on success. Edge case §12 row 12's "no server-side merge UI" is hereby closed. No MCP tool (admin surface).

**E2E catch-up.** New Playwright scenarios against the real booted server (the §19 harness): v0.8 — add an annotation through the UI and see it in the timeline; a seeded fix attempt renders its state pill and commit link; a seeded spiking issue shows the badge; repoUrl round-trips through settings. v0.9 — release health renders from seeded events + usage; an http monitor is created and shows a probe status; merging an issue moves its events and the source shows the merged state. Suite grows 8 → ≥15.

**Deployment (the box).** After merge + box upgrade: no new env vars; migration 0008 applies on restart; `js-dist` refresh to 0.6.0 and consumer re-vendors (which also finally pick up the v0.7 uploader 429-backoff and the v0.8 commit wiring); consider `consoleBreadcrumbs: true` + usage-release attribution in the four consumer apps' init calls; an `ops` http monitor for https://err.autogeny.ai itself is the natural first uptime probe (external-perspective probing of the box FROM the box is limited — note it, don't oversell it). Box-side only; not acceptance-gated.

**Acceptance.** Migration 0008 applies cleanly to a v0.8 database. Console breadcrumbs: hooked console calls emit capped breadcrumbs, the re-entrancy guard survives a console call inside `JSON.stringify` (a toJSON that logs), `close()` restores the originals, and a captured exception's envelope carries the recorded trail. Usage: events with a valid `release` store it, invalid release drops the event not the batch, the client stamps automatically. Release health: seeded multi-release fixtures produce deterministic rows, ratio `null` without pageviews, ≤20 rows, ordering asserted. Uptime: an http monitor probes on schedule (injected clock + stubbed fetch), flips `missed` only on the second consecutive failure, recovers on first success, dispatches both webhooks exactly once per episode, respects timeout and the 5-probe cap, rejects SSRF targets at save and probe time, and check-in pings 409. Merge: events/annotations/fix attempts move atomically, alias routing sends the next ingest to the target, counts recompute, the merged source is hidden from default lists and un-PATCHable, merge-into-merged 400s. E2E: the new scenarios pass in CI. All §15 gates green; v0.8 behavior otherwise byte-identical.
