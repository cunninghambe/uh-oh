...
**Usage release attribution.** `usage_events` gains nullable `release` (migration 0008); usage ingest accepts an optional per-event `release` string (≤128 chars, invalid → event dropped not batch); the client stamps its `init` release on every usage event automatically. The summary endpoint is unchanged; release health consumes it.

**Release health.** `GET /api/projects/:id/release-health?days=N` (N 1..90, default 30; JWT, read token, agent token) → `{ releases, totals }`. `releases`: every release with events in the window (≤20, ordered `lastEventAt` desc), each `{ id, version, build, platform, commitSha, events, fatalEvents, distinctIssues, firstEventAt, lastEventAt, pageviews, crashesPer1kPageviews }` — `pageviews` counts usage pageviews attributed to that release; `crashesPer1kPageviews` = `events / pageviews × 1000` rounded to 1 decimal, `null` when `pageviews` is 0 (analytics off, non-web, or unattributed). `totals`: the same trio project-wide. Deterministic SQL over existing indexes. MCP tool `get_release_health` (scope `read`; tool count 17 → 18). Dashboard: a Release health section on the project page (table, ratio badges, hidden gracefully when the endpoint is absent — the §21 pattern).
