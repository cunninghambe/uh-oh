...
describe('sendEvent', () => {
    it('returns ok:true on 202', async () => {
        ...
        expect(fetchFn).toHaveBeenCalledWith('https://errors.example.com/ingest/mykey', expect.objectContaining({ method: 'POST' }));
    });
    ...
    it('returns ok:false on timeout (AbortError)', async () => {
        const fetchFn = vi.fn().mockImplementation((_url, opts) => new Promise((_resolve, reject) => {
            if (opts.signal) {
