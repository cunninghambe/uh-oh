...
// ── Formatting helpers ────────────────────────────────────────────────────────
const toIso = (ms) => ms == null ? undefined : new Date(ms).toISOString();
...
const formatFrames = (raw, resolved) => {
    const rawFrames = raw ?? [];
...
const formatBreadcrumbs = (rows, cap = 20) => {
...
const formatFullEvent = (e, resolved) => {
    const env = parseEnvelope(e.payload);
...
const resolveProjectId = async (backend, ref) => {
 ...
 */
export const registerUhOhTools = (server, backend) => {
    ...
    server.registerTool('create_project', {
        ...
        description: 'Create a new project. Returns the project with its generated slug and public key.',
        inputSchema: { name: z.string().min(1).max(128) },
        annotations: WRITE,
    }, (args) => run(async () => ok({ project: formatProject(await backend.createProject({ name: args.name })) })));
    server.registerTool('update_project', {
        title: 'Update project',
        description: 'Update a project name, webhook URL (SSRF-validated by the server; a rejected URL is returned as a tool error) or alert dedupe minutes. Pass webhookUrl null to clear it.',
        inputSchema: {
    ...
    server.registerTool('list_issues', {
        title: 'List issues',
        description: 'List issues for a project (by id or slug), optionally filtered by status (open, resolved, ignored, or the system-set regressed) and sorted. Returns issues plus the total count.',
        inputSchema: {
    ...
    server.registerTool('get_issue', {
        title: 'Get issue',
        description: 'Get an issue with a summary of its latest event, symbolicated stack frames when available, and its recent breadcrumbs (last 20).',
    ...
    server.registerTool('get_event', {
        title: 'Get event',
        description: 'Get a single event as a cleaned-up envelope (noise dropped) with resolved stack frames. Set symbolicate false to skip symbolication.',
        inputSchema: {
        ...
        if (!detail)
            throw new BackendError('event not found', { code: 'not_found', status: 404 });
        return ok({
            event: formatFullEvent(detail.event, detail.frames),
            ...formatBreadcrumbs(detail.breadcrumbs),
        });
    }));
    server.registerTool('set_issue_status', {
        title: 'Set issue status',
        description: "Set an issue status to open, resolved or ignored. The 'regressed' status is system-set (a resolved issue that received a new event) and cannot be set here; PATCH a regressed issue to resolved to re-arm regression detection.",
    ...
    server.registerTool('list_releases', {
        title: 'List releases',
        description: 'List releases for a project (by id or slug), including whether ProGuard mapping and Hermes source map symbols have been uploaded.',
    ...
    server.registerTool('get_server_health', {
        ...
        description: 'Report server health (/healthz) plus a parsed subset of /metrics: total events ingested, new issues, and permanent webhook failures.',
        annotations: READ,
    }, () => run(async () => {
    ...
    server.registerTool('list_top_issues', {
        title: 'List top issues',
        description: 'List open and regressed issues across ALL projects, ranked by event volume within the last N days. Each carries its project slug, platform, and windowed + all-time counts.',
        inputSchema: {
    ...
    server.registerTool('list_monitors', {
        title: 'List monitors',
        description: 'List check-in monitors, optionally scoped to one project (by id or slug). Each includes its cadence, status (ok, missed, or paused), last check-in, and a computed overdue flag.',
        inputSchema: { project: z.string().min(1).optional() },
        annotations: READ,
    }, (args) => run(async () => {
        const input = args.project !== undefined
    ...
    server.registerPrompt('fix_crash', {
        title: 'Fix a crash',
        description: 'Diagnose and fix a specific uh-oh issue by pulling its bundle and patching the offending code.',
...
export const createUhOhMcpServer = (backend) => {
