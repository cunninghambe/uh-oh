...
export const registerApiRoutes = (app: FastifyInstance, db: Db): void => {
  app.get('/api/projects', () => ({ projects: listProjects(db) }));

  app.post<{ Body: unknown }>('/api/projects', (req, reply) => {
    const body = req.body;
  ...
  app.patch<{ Params: { id: string }; Body: unknown }>('/api/projects/:id', (req, reply) => {
    const body = req.body;
    if (typeof body !== 'object' || body === null) {
      return reply.code(400).send({ error: 'invalid_body' });
    }
  ...
  app.post<{ Params: { id: string } }>('/api/projects/:id/rotate-key', (req, reply) => {
  ...
  }>('/api/projects/:id/issues', (req, reply) => {
  ...
  app.get<{ Params: { id: string } }>('/api/issues/:id', (req, reply) => {
  ...
  app.patch<{ Params: { id: string }; Body: unknown }>('/api/issues/:id', (req, reply) => {
  ...
  }>('/api/issues/:id/events', (req, reply) => {
    const issue = getIssue(db, req.params.id);
    if (!issue) return reply.code(404).send({ error: 'not_found' });
    const limit = req.query.limit ? Math.max(1, Math.min(200, Number(req.query.limit))) : 50;
    const offset = req.query.offset ? Math.max(0, Number(req.query.offset)) : 0;
  ...
  app.get<{ Params: { id: string } }>('/api/events/:id', (req, reply) => {
